...
/**
 * avl_insert - function inserts a value into an AVL tree
 * @tree: it's double pointer to the root node of the AVL tree to insert into
...
/**
 * r_insert_node - function recursively inserts a node value into an AVL tree
 * @tree: it's a double pointer to the root node of the AVL tree struct
 * @parent: it's the parent node of the current AVL node
 * @new: it's a double pointer to the newly inserted node
 * @nval: The value to be inserted into the AVL tree
 *
 * Return: Pointer to the new root after insertion, otherwise NULL on failure
 */
